...
var FTColumnflow = (function () {
	...
	var
		...
		showGridStyles = '#[targetId] .[pageClass] { background-image: -webkit-linear-gradient(rgba(0, 0, 255, 0.5) .1px, transparent 1px), linear-gradient(rgba(0, 0, 255, 0.8) .1px, transparent 1px); background-size: 100% [lh]px, 100% [lhd]px; background-origin: content-box, content-box; }',
	...
	function FTColumnflow(target, viewport, userConfig) {
		...
		function _setConfig(userConfig) {
			...
			// Enable Copyfit?
			if (config.lineCount !== undefined) lineCount = config.lineCount;
			...
			if ((config.lineHeight !== null) && isNaN(config.lineHeight = parseFloat(config.lineHeight, 10))) {
...
//console.log("anchorY: ", elementDefinition.anchorY, "Height: ", normalisedElementHeight);
...
//console.log("elementTopPos: ", elementTopPos, "topSplitPoint: ", topSplitPoint, "bottomSplitPoint: ", bottomSplitPoint.toFixed(2), "normalisedElementHeight: ", normalisedElementHeight, "elementBottomPos: ", elementBottomPos )
		...
		async function _flowContent() {
			...
			await _createPageObjects(indexedPageNum);
			...
			workingColumnFrag = workingColumn.fragments[indexedColumnFrag];
			 ...
			 if (!workingColumnFrag) {
				await _advanceWorkingColumnFragment();
			}
	...
	//////////////////////////////
	//Copyfit mit lineCount jrgdrs

	function copyfit(){

		/* determines a target result value based on tests of function CFCheck with oscillating input values */
		function _CFOscillate(targetResult){
			var hiMark=1, lowMark=0, actualResult=0, inputValue=0, i=0, maxTestCnt=14;
			while(actualResult!=targetResult && i<maxTestCnt){
				i++; inputValue=(hiMark+lowMark)/2;
				actualResult = _CFCheck(inputValue);
				//console.log(inputValue,actualResult,targetResult,hiMark,lowMark);
				actualResult>targetResult ? hiMark=inputValue : lowMark=inputValue ;
			}
			actualResult!=targetResult ? console.log( '*** ERROR *** CF accuracy ' + actualResult + '/' + targetResult ) : 0 ;
			return inputValue
		}

		function _CFCheck(cfFactor){

					// just for legacy purposes
					var CFemval = (-1 + 2 * cfFactor);//.toFixed(8);
					CFroot.style.setProperty("--ws", CFemval + "em"); 

			CFroot.style.setProperty("--cfFactor", cfFactor ); 
			var rs = getComputedStyle(CFroot);
			return (document.getElementById("autoheight").getBoundingClientRect().height / config.lineHeight).toFixed(0)
		}

		var CFroot = document.querySelector(":root");
		var cfFactor = _CFOscillate(lineCount);
		var emval = (-1 + 2 * cfFactor);//.toFixed(8);
		console.log('CF result factor: ', cfFactor );
	...
	function _addFlowedElement(element, index) {
		...
		function _round(val) {
			return Math.round(val * 100000) / 100000; //JD
		}
		...
		this.flow = async function(flowed, fixed) {
